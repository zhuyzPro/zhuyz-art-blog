...
hexo.extend.filter.register('before_post_render', data => {
  ...
  if (hexo.config.post_asset_folder) {
    if (typeof topImg === 'string' && topImg.indexOf('/') === -1 && imgTestReg.test(topImg)) data.top_img = `${data.path}${topImg}`
    if (typeof coverVal === 'string' && coverVal.indexOf('/') === -1 && imgTestReg.test(coverVal)) data.cover = `${data.path}${coverVal}`
  }
